...
input=Table/03_16S_exSTD_seq.fasta
out=Table
mkdir -p ${out}/code

seqtab=Table/03_16S_excludeSTDmatrix.rds
####################################################################
for i in 0.98
do
    
vsearch --cluster_fast $input --id ${i} --mothur_shared_out ${out}/04_ASV_OTU_corestab_${i}.txt --centroids ${out}/04_OTUseq_${i}.fasta --msaout ${out}/04_seqAlign_${i}.txt
...
library(seqinr)
otu <- read.table("Table/04_ASV_OTU_corestab_${i}.txt",
                  header=TRUE, row.names=2)[,-c(1:2)]
seqtab <- readRDS("Table/03_16S_excludeSTDmatrix.rds")
