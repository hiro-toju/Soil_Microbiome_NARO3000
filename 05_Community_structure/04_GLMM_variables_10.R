...
stat <- sml[,c(colnames(sml)[continuous], 'experimental_purpose')] ##
stat[,-ncol(stat)] <- apply(stat[,-ncol(stat)], 2, scale)
colnames(stat)[ncol(stat)] <- 'rand'
...
summary(lme4::glmer(DL.bin ~ scale(bac.shannon) + scale(fng.shannon) + scale(BbyFabundance) + (1| rand), family = binomial,  data=df))

summary(lme4::glmer(DL.bin ~ scale(pH_dry_soil) + scale(EC_electric_conductivity) + scale(BbyFabundance) + (1| rand), family = binomial,  data=df))
...
#summary(lme4::glmer(DL.bin~ BbyFabundance + (1| rand), family = binomial,  data=df))

df2 <- na.omit(df[, c('BbyFabundance', 'DL.bin', 'rand')])
df3 <- matrix(NA, nrow=0, ncol=3)

for(i in 1:length(unique(df2$rand))) {
	a <- subset(df2, df2$rand==unique(df2$rand)[i])
	b <- nrow(a)
...
g <- ggplot(df3, aes(x= BbyFabundance, y=DL.bin)) + geom_point(shape=3) + facet_wrap(~ rand, ncol=3)
...
for (i in 1:length(listrand)) {
	glmres[[i]] <- glm(DL.bin ~ BbyFabundance, family = binomial,  data=subset(df3, df3$rand== listrand[i]))
