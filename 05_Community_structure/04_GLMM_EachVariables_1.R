############################################################################
####
#### R script for Fujita (2022)
####
#### Removing outlier from data
#### 2022.03.16 Fujita
#### 2022.06.23 Toju
#### R 4.1.2
#### 
#### 
############################################################################

set.seed(123)
setwd("../")

DLth <-15

## -- Loading Function and Library
library('AnalysisHelper')
load.lib(c('ggplot2', 'tidyr', 'vegan', "ggmap", "maps",'cowplot', 'lmerTest' ,'RColorBrewer', 'dplyr'))
library(igraph)
library(lme4)
library(ggeffects)
library(tidyverse)
library(MASS)
library(foreach)

# -- Create directory to save
dir <- make.dir('05_Community_structure/04_output')

## |||||||||||||||||||||||||||||||||||||||||||||||||||||||||| ##
# -- Load data table

## -- Converted
sml <- readRDS("Table/0301_sml_envpca.rds") 

sml$PC2[sml$PC2>5] <- NA
index <- as.data.frame(readRDS('Table/0303_communityIndices_ReadCount.rds'))

############################################################################

classV <- c()
for(i in 1:ncol(sml)){ classV <- c(classV, class(sml[,i])) }
continuous <- which(classV=='numeric')
factor <- which(classV=='character')

## -- Used for test missing sample less than 30% of no.samples
statdf <- sml[,continuous]

statdf <- na.omit(statdf[,apply(statdf, 2, function(X){ sum(is.na(X))})/nrow(statdf) < 0.3])

## ============================================= ##

## -- Correlation between variables
cor.mat <- cor(statdf, index[rownames(statdf), ], use='pairwise.complete.obs',
               method='spearman')
#pairs(statdf)

lf.cor <- gather(cbind(variable=rownames(cor.mat), as.data.frame(cor.mat)),
				 key=key,value=value,-1)
				 
ggcor <- ggplot(lf.cor)+
		 geom_tile(aes(x= key, y= variable, fill=value), color='grey30')+
		 scale_fill_gradient2( low='deeppink3', limits=rep(max(abs(lf.cor$value)),2)*c(-1, 1) )+
		 coord_equal()+
         theme_minimal()+
		 theme(axis.text.x=element_text(angle=90, hjust=1))
pdf(sprintf('%s/correlation_between_variables.pdf', dir$figdir))
plot(ggcor)
dev.off()

## ============================================= ##
## -- Statics

stat <- sml[,c(colnames(sml)[continuous], 'experimental_purpose', 'Month', 'crop')] ##
stat[,-c((ncol(stat)-2):ncol(stat))] <- apply(stat[,-c((ncol(stat)-2):ncol(stat))], 2, scale)

DL.bin <- sml$DL
DL.bin[which(DL.bin != 'Level 1')] <- 0
DL.bin[which(DL.bin == 'Level 1')] <- 1
mode(DL.bin) <- 'numeric'

df <- cbind(stat, index[rownames(stat), ], DL.bin)
df$bac.total.abundance <- log(df$bac.total.abundance)
df$fng.total.abundance <- log(df$fng.total.abundance)
df$bac.total.abundance[is.infinite(df$bac.total.abundance)] <- NA

#sink(sprintf('%s/abundance_glm.txt', dir$tabledir))
Sample.ID <- rownames(df)
out <- cbind(Sample.ID, df)
write.table(out, file=sprintf('%s/indexes.txt', dir$tabledir), quote=F, sep="\t", row.names=F)
saveRDS(df, file=sprintf('%s/indexes.rds', dir$rdsdir))
saveRDS(df, file='Table/indexes.rds')

# odd distribution found in Total_C,  rate_of_chemical_fertilizer_applicationN,P,K

########################################################
# GLMM

res <- list()

res[[1]] <- lme4::glmer(DL.bin ~ scale(pH_dry_soil) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df, control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))

res[[2]] <- lme4::glmer(DL.bin ~ scale(EC_electric_conductivity) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df, control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))

res[[3]] <- lme4::glmer(DL.bin ~ scale(available_P) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df, control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))

res[[4]] <- lme4::glmer(DL.bin ~ scale(CNratio) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df, control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))

res[[5]] <- lme4::glmer(DL.bin ~ scale(BbyFabundance) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df, control=glmerControl(optimizer="nlminbwrap", calc.derivs = FALSE, optCtrl = list(maxfun = 10000, maxiter=100000)))

sink(sprintf('%s/04_GLMM_EachVariables.results_selected.txt', dir$tabledir))
foreach(i=1:5) %do% {summary(res[[i]])}
sink()



#r2 <- lme4::glmer(DL.bin ~ scale(bac.shannon) + scale(fng.shannon) + scale(BbyFabundance) + (1| experimental_purpose) + (1| Month) + (1| crop), family = binomial,  data=df)


