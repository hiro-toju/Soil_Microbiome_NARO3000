...
seq <- read.fasta("Table/02_ITSmerge_clus_seq.fasta")
seqtab <- readRDS("Table/02_ITSmerge_seqtab.rds")

# -- Reference database
reference="Table/STD_fng.fasta"
...
##############################################
alignlen <- blast[which(blast[,4] > 200 & blast[,3] > 95),]

STDseq <- seq[names(seq) %in% unique(alignlen[,1])]
exSTDseq <- seq[-which(names(seq) %in% unique(alignlen[,1]))]

write.fasta(STDseq, names(STDseq), "Table/03_ITS_STD_seq.fasta" )
write.fasta(exSTDseq, names(exSTDseq), "Table/03_ITS_exSTD_seq.fasta" )

exstdmat <- seqtab[,-which(colnames(seqtab) %in% names(STDseq))]

saveRDS(exstdmat, "Table/03_ITS_excludeSTDmatrix.rds")
...
saveRDS(stdmerege, "Table/03_ITS_STDmatrix.rds")
