...
crop.row <- rownames(sml)[which(!is.na(sml$crop))]
commonrow <- sort(intersect(intersect(rownames(read16s), rownames(readits)), crop.row), decreasing=FALSE)
dflist <- cbind(read16s[commonrow,], readits[commonrow,])
cropcategory <- data.frame(crop=sml[commonrow,'crop'], row.names = commonrow)
...
## |||||||||||||||||||||||||||||||||||||||||||||||||||||||| ##
orgntmp <- t(Taxa.mat(t(networkMat), cropcategory, 'crop', mean))
...
Merge <- foreach::foreach(x=randlabel)%dopar%{ 
    tmp <-  t(Taxa.mat(t(x), cropcategory, 'crop', mean))
...
saveRDS(join, 'Table/08_01_preference_to_crop.rds')
## |||||||||||||||||||||||||||||||||||||||||||||||||||||||| ##
...
ggsave(plot=ggDL, filename=sprintf('%s/Preference_to_cropcategory.pdf', dir$figdir),w=5, h=15)
