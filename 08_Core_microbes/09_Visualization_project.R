...
x <- readRDS('Table/08_01_preference_to_project.rds')
...
ggsave(plot=plot_grid(plotlist=ggDL, ncol=3, align='v'),
       filename=sprintf('%s/Specificity_to_project_label.pdf', dir$figdir), w=15, h=20)
...
ggsave(plot=plot_grid(plotlist=ggDL, ncol=3, align='v'),
       filename=sprintf('%s/Specificity_to_project.pdf', dir$figdir), w=15, h=20)
