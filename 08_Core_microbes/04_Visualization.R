...
gobj[[1]] <- ggplot(ggnet, aes(x=x, y=y, xend=xend, yend=yend))+
...
gobj[[2]] <- ggplot(ggnet, aes(x=x, y=y, xend=xend, yend=yend))+
        geom_edges(color='grey80', size=0.3)+
        #geom_edges(data=X[X$bb=='bone',], color='green3', size=1)+
...
gobj[[3]] <- ggplot(ggnet, aes(x=x, y=y, xend=xend, yend=yend))+
        geom_edges(color='grey80', size=0.3)+
        #geom_edges(data=X[X$bb=='bone',], color='green3', size=1)+
        geom_nodes(aes(x=x, y=y, fill=as.factor(module), size=score), shape=21)+
        scale_fill_manual(values=colvec)+
...
ggsave(plot=plot_grid(plotlist=gobj, ncol=1, align='v') ,
       filename=sprintf('%s/Network_Kingdom_Betweenness_Eigenvector.pdf', dir$figdir), w=8, h=14)
...
zv <- as.numeric(summaryTab2[V(mbP)$name, 'z.degree'])
V(mbP)$score <- (zv - min(zv, na.rm=TRUE))/(max(zv, na.rm=TRUE) - min(zv, na.rm=TRUE))
V(mbP)$score[is.na(V(mbP)$score)] <- 0

ggnet <- ggnetwork(mbP, layout=lay)

zC[[1]] <- ggplot(ggnet, aes(x=x, y=y, xend=xend, yend=yend))+
  geom_edges(color='grey80', size=0.3)+
  #geom_edges(data=X[X$bb=='bone',], color='green3', size=1)+
  geom_nodes(aes(x=x, y=y, fill=as.factor(module), size=score^2), shape=21)+
  scale_fill_manual(values=colvec)+
  theme_void()+
  theme(legend.box = "horizontal")+
  coord_equal()+
...
V(mbP)$score <- as.numeric(summaryTab2[V(mbP)$name, 'moduleConect'])
#V(mbP)$score[V(mbP)$score<0] <- 0

ggnet <- ggnetwork(mbP, layout=lay)

zC[[2]] <- ggplot(ggnet, aes(x=x, y=y, xend=xend, yend=yend))+
  geom_edges(color='grey80', size=0.3)+
  #geom_edges(data=X[X$bb=='bone',], color='green3', size=1)+
  geom_nodes(aes(x=x, y=y, fill=as.factor(module), size=score^3), shape=21)+
  scale_fill_manual(values=colvec)+
  theme_void()+
  theme(legend.box = "horizontal")+
  coord_equal()+
...
ggsave(plot=plot_grid(plotlist=zC, ncol=2, align='v') ,
       filename=sprintf('%s/withinModule_AmongModule.pdf', dir$figdir), w=15, h=8)
