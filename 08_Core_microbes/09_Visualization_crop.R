...
x <- readRDS('Table/08_01_preference_to_crop.rds')
...
ggsave(plot=plot_grid(plotlist=ggDL, ncol=3, align='v'),
       filename=sprintf('%s/Specificity_to_crop_label.pdf', dir$figdir), w=15, h=20)
...
ggsave(plot=plot_grid(plotlist=ggDL, ncol=3, align='v'),
       filename=sprintf('%s/Specificity_to_crop.pdf', dir$figdir), w=15, h=20)
